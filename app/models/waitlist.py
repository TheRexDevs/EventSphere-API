...
class Waitlist(db.Model):
    ...
    waitlist_time = db.Column(db.DateTime(timezone=True), default=DateTimeUtils.aware_utcnow)
    status = db.Column(db.String(20), nullable=False, default="waiting")  # waiting, confirmed, cancelled
    
    event = db.relationship('Event', backref=db.backref('waitlists', lazy='dynamic'))
    student = db.relationship('AppUser', backref=db.backref('waitlists', lazy='dynamic'))
